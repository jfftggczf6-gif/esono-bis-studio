...
CREATE POLICY "Users see modules of own enterprises" ON public.enterprise_modules FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM enterprises e WHERE e.id = enterprise_modules.enterprise_id AND (e.user_id = auth.uid() OR e.coach_id = auth.uid())));
CREATE POLICY "Users can insert modules for own enterprises" ON public.enterprise_modules FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM enterprises e WHERE e.id = enterprise_modules.enterprise_id AND (e.user_id = auth.uid() OR e.coach_id = auth.uid())));
CREATE POLICY "Users can update modules of own enterprises" ON public.enterprise_modules FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM enterprises e WHERE e.id = enterprise_modules.enterprise_id AND (e.user_id = auth.uid() OR e.coach_id = auth.uid())));
...
CREATE POLICY "Users see deliverables of own enterprises" ON public.deliverables FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM enterprises e WHERE e.id = deliverables.enterprise_id AND (e.user_id = auth.uid() OR e.coach_id = auth.uid())));
CREATE POLICY "Users can insert deliverables for own enterprises" ON public.deliverables FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM enterprises e WHERE e.id = deliverables.enterprise_id AND (e.user_id = auth.uid() OR e.coach_id = auth.uid())));
CREATE POLICY "Users can update deliverables of own enterprises" ON public.deliverables FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM enterprises e WHERE e.id = deliverables.enterprise_id AND (e.user_id = auth.uid() OR e.coach_id = auth.uid())));
